(function () {
  ...
  function getAllPaths(router) {
    ...
    Docsify.dom
    .findAll('.sidebar-nav a:not(.section-link):not([data-nosearch]), .app-nav a')
  ...
  function init(config, vm) {
    ...
    if (paths.length && isAuto && config.pathNamespaces) {
      ...
      var isExistReadme = paths.indexOf(namespaceSuffix + '/HOME') === -1;
      if (isExistHome && isExistReadme) {
        paths.unshift(namespaceSuffix + '/');
      }
    } else if (paths.indexOf('/') === -1 && paths.indexOf('/HOME') === -1) {
      paths.unshift('/');
    }
